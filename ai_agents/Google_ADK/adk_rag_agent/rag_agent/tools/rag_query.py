"""
Tool for querying Vertex AI RAG corpus and retrieving relevant information.
"""
...
def rag_query(
    ...
    """
    Query a Vertex AI RAG corpus with an user question and return relevant information.
    ...
    try:
        ...
        if not check_corpus_exists(corpus_name=corpus_name, tool_context=tool_context):
            return {
                "status": "error",
                "message": f"Corpus '{corpus_name}' does not exist. Please create it first using the create_corpus",
                "query": query,
        ...
        # Configure the corpus resource name
        ...
        if hasattr(response, "contexts") and response.contexts:
            for ctx_group in response.contexts.contexts:
                result = {
                    ...
                    "source_name": (
                        ctx_group.source_display_name if hasattr(ctx_group, "source_display_name") else ""
                    ),
