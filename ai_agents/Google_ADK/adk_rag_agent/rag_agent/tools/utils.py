...
def get_corpus_resource_name(corpus_name: str) -> str:
    ...
    Handle various input formats and ensure the returned name follow Vertex AI's requirements.  

    Args:
        corpus_name (str): The corpus name or display name
    Returns:
        str: The resource name of the corpus
    """

    logger.info(f"Getting resource name for corpus: {corpus_name}")

    # `corpus_name` is already a full resource name with the project/locations/ragCorpora format
...
def set_current_corpus(corpus_name: str, tool_context: ToolContext) -> bool:
    ...
    Args:
        corpus_name (str): The name of the corpus to set as the current
        tool_context (ToolContext): The tool context for state management
    Returns:
        bool:  True if the corpus exists and was set as the current, False otherwise
    """

    # Check if the corpus exist first
    if check_corpus_exists(corpus_name=corpus_name, tool_context=tool_context):
        tool_context.state["current_corpus"] = corpus_name
        return True
    return False