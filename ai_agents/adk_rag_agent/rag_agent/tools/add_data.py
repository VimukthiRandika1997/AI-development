...
def add_data(
    ...
    Args:
        corpus_name (str): The name of the corpus ot add data to. If empty, the current corpus will be used.
        path (List[str]): List of URLs or GCS paths to add to the corpus.
                Supported formats:
    ...
    if not check_corpus_exists(corpus_name=corpus_name, tool_context=tool_context):
        return {
            "status": "error",
            "message": f"Corpus '{corpus_name}' does not exist. Please create it first before using the create_corpus tool.",
    ...
    # Preprocess paths to validate and convert Google Docs URLs to Drive format if needed
    validated_paths = []
    invalid_paths = []
    conversions = []
    ...
    try:
        ...
        corpus_resource_name = get_corpus_resource_name(corpus_name=corpus_name)

        # Set up chuncking configuraiton
        transformation_config = rag.TransformationConfig(
            chunking_config=rag.ChunkingConfig(chunk_overlap=DEFAULT_CHUNK_OVERLAP, chunk_size=DEFAULT_CHUNK_SIZE)
        )
        ...
        # Set this as the current corpus if not alread set
        ...
        return {
            "status": "success",
            "message": f"Successfully added {import_result.imported_rag_files_count} files to corpus '{corpus_name}' {conversion_msg}",
            "corpus_name": corpus_name,
